...
bot.on("message", async message => {
    ...
    if(cmd === `${prefix}wl24`) {
    return sendMsg(`https://wii.guide/wiilink24`);
    }
    ...
    if(cmd === `${prefix}more`) {
    return sendMsg(`https://wii.guide/site-navigation\nhttps://bloodythorn.github.io/wiihacks-wiki/`);
    }

    if(cmd === `${prefix}pins`) {
    return sendMsg(`Please check the pinned messages for more guides`);
    }

    if(cmd === `${prefix}sega`) {
    return sendMsg(`https://bloodythorn.github.io/wiihacks-wiki/tutorials/nintendont.html`);
    }

    if(cmd === `${prefix}credits`) {
    return sendMsg(`Bot creator:\nakisblack#2545\n\nKudos:\nshrek#7532 aka DerpMaster2\nWiiMaster#7071 aka WiiExpertise\nMethodOrMadness#9199 aka Method\nDevnol#9366 aka CommandBlock6417\noscie#1093 aka nothing\nand many others`);
    }

    if(cmd === `${prefix}vwii`) {
    return sendMsg(`https://wiiu.hacks.guide/#/vwii-modding`);
    }
    
    if(cmd === `${prefix}forwarders`) {
    return sendMsg(`https://wii.guide/wiigsc`);
    }

    if(cmd === `${prefix}root`) {
    return sendMsg(`https://imgur.com/a/HDCWvt0`);
    }

    if (cmd === `${prefix}ping`) {
    var ping = Date.now() - message.createdTimestamp + " ms";
    return sendMsg("Pong! The bot's ping is " + `${ping}`);
    }

    if(cmd === `${prefix}ios`) {
    return sendMsg(`I am on life support, leave me alone`);
    }
